...
export const UpdateProduct = ({ inputs, title }) => {
    ...
    <div className="new">
      ...
      <div className="newContainer">
        ...
        <div className="bottom">
          <div className="left"><h2>Id: {_id}</h2>
          ...
          <div className="right">
            <form onSubmit={async (e) => {
                console.log("submit")
                e.preventDefault();
                const res = await updateRequest(`/admin/laptops/${_id}`, handleFormData());
                if(res.success == false) {
                  setModalShow(true)
                  setMessage({
                    heading: "Sửa thông tin không thành công",
                    center: "Error",
                    message: res.message
                  })
                } else {
                  setModalShow(true)
                  setMessage({
                    heading: "Sửa thông tin thành công",
                    center: "Success",
                    message: "Sửa thành công thành công " + res.data._id
                  })
                }
               }}>
              ...
              <div className="formInput" >
                ...
                <div className="giftInput" >
                  ...
                  <a onClick={ () => { const gift = { title: imgTitle, img: URL.createObjectURL(img) }; setItem((prev) =>  [...prev, gift]); setImgTitle(""); setImg("") } }>Add</a>
                  <Gift gifts={item} />
                </div>
                <div className="formInput" >
                  <label>Brand</label>
              ...
              <input type="submit" value="Update Laptop"></input>
            </form>
          </div>
        </div>
      </div>
